...
def train(net, start):
    ...
    for iteration in range(net.num_iterations):
        ...
        if iteration % 25000 == 0:
            torch.save(net, "DQNmodel_weights/current_model_" + str(iteration) + ".pth")
        ...
        if terminal:
            ...
            if (meant>300) & (t>600):
                ...
                np.save('finalresults/DQNfinal.npy',[iteration,time.time() - start])
                save_durations(net.episode_durations)
                torch.save(net, "DQNmodel_weights/final_model.pth")
                break
...
def save_durations(episode_durations):
    ...
    np.save("duration/DQNduration.npy", durations_t.numpy())
...
if __name__ == "__main__":
    ...
    if mode == 'test':
        model = torch.load(
            ...
            'DQNmodel_weights/final_model.pth',
        ...
        if not os.path.exists('DQNmodel_weights/'):
            os.mkdir('DQNmodel_weights/')
