...
def train(net, start):
    ...
    for iteration in range(net.num_iterations):
        ...
        if iteration % 25000 == 0:
            torch.save(net, "PriDQNmodel_weights/current_model_" + str(iteration) + ".pth")
        ...
        if terminal:
            ...
            if (meant>300) & (t>600):
                ...
                np.save('finalresults/PriDQNfinal.npy',[iteration,time.time() - start])
                save_durations(net.episode_durations)
                torch.save(net, "PriDQNmodel_weights/final_model.pth")
                break
...
def save_durations(episode_durations):
    ...
    np.save("duration/PriDQNduration.npy", durations_t.numpy())
...
if __name__ == "__main__":
    ...
    if mode == 'test':
        model = torch.load(
            ...
            'PriDQNmodel_weights/final_model.pth',
        ...
        if not os.path.exists('PriDQNmodel_weights/'):
            os.mkdir('PriDQNmodel_weights/')
