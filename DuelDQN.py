...
def train(net, start):
    ...
    for iteration in range(net.num_iterations):
        ...
        if iteration % 25000 == 0:
            torch.save(net, "DuelDQNmodel_weights/current_model_" + str(iteration) + ".pth")
        ...
        if terminal:
            ...
            if (meant>300) & (t>600):
                ...
                np.save('finalresults/DuelDQNfinal.npy',[iteration,time.time() - start])
                save_durations(net.episode_durations)
                torch.save(net, "DuelDQNmodel_weights/final_model.pth")
                break
...
def save_durations(episode_durations):
    ...
    np.save("duration/DuelDQNduration.npy", durations_t.numpy())
...
if __name__ == "__main__":
    ...
    if mode == 'test':
        model = torch.load(
            ...
            'DuelDQNmodel_weights/final_model.pth',
        ...
        if not os.path.exists('DuelDQNmodel_weights/'):
            os.mkdir('DuelDQNmodel_weights/')
